...
automl <- function(train, y, valid = NULL, test = NULL, x = NULL, id.feats = NULL, time.partition.feature = NULL,
  ...
  pp$pipeline$settings$ml.data.leakage.cutoff <- info$data.leakage.cutoff
  ...
  pp$pipeline$settings$ml.models.explored <- length(models)
  pp$pipeline$settings$ml.model.search.time <- automl.search.max.runtime.mins
  if(is.null(info$pipelines.explored) == TRUE){
  ...
  if(is.null(info$pipelines.explored) == TRUE){
    pp$pipeline$settings$ml.pipelines.explored <- 0
  } else {
    pp$pipeline$settings$ml.pipeline.search <- info$pipelines.explored
  }
  ...
  if(is.null(info$pipelines.explored) == FALSE){
